...
describe Temporal::Metadata do
  ...
  describe '.generate_activity_metadata' do
    subject { described_class.generate_activity_metadata(data, namespace, config.converter) }
  ...
  context '.generate_workflow_metadata' do
    subject { described_class.generate_workflow_metadata(event, task_metadata, config.converter) }
